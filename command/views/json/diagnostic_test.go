...
`),
  ...
  var.k,
					...
					StartLine:          (1),
					HighlightStartByte: (0),
					HighlightEndByte:   (102),
					Values: []DiagnosticExpressionValue{
						{
							Traversal: `var.a`,
							Statement: `is true`,
	...
	for name, tc := range testCases {
		...
		t.Run(fmt.Sprintf("golden test for %s", name), func(t *testing.T) {
			...
			if !cmp.Equal(wantBytes, gotBytes) {
				t.Fatalf("wrong result\n:%s", cmp.Diff(wantBytes, gotBytes))
			}
		})
	}
}
