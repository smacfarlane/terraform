...
type DiagnosticSnippet struct {
	...
	// HighlightStartByte is the byte offset into Code at which the diagnostic
	// source range starts, which ought to be highlighted as such by the
	// consumer of this data.
	HighlightStartByte int `json:"highlight_start_byte"`

	// HighlightEndByte is the byte offset into Code at which the diagnostic
	// source range ends.
	HighlightEndByte int `json:"highlight_end_byte"`
...
func NewDiagnostic(diag tfdiags.Diagnostic, sources map[string][]byte) *Diagnostic {
	...
	if sourceRefs.Subject != nil {
		...
		if src != nil {
			...
			diagnostic.Snippet.HighlightStartByte = start
			diagnostic.Snippet.HighlightEndByte = end
